...
import hook from '../../utils/musicHook';
...
export default async (msg: Message) => {
    ...
    if (ytdl.validateID(input) || ytdl.validateURL(input)) {
        try {
            ...
            if (l > 3600)
                return msg.channel.send("longer than 1h");
        ...
        try {
            ...
            for(let v in pl.items) {
                let i = pl.items[v];
                if (i.duration !== null) {
                    let length = i.duration.split(':').reverse().reduce((prev, curr, i) => prev + Number(curr) * Math.pow(60, i), 0);
                    if (length < 3600) {
            ...
            if (count === 0)
                return msg.channel.send("cannot add anyone video from playlist");
        } catch(_) {
    ...
    if (info !== null && d.isPlaying)
        msg.channel.send('added to queue');
    
    set(msg.guild.id, d);

    if (d.queue.length === 1 || !d.isPlaying)
        hook(msg, true);
