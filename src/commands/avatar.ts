...
const avatarCommand = (msg: Message, mentionedUsers?: Collection<string, User>) => {
  ...
  if (mentionedUsers.size === 0) {
    avatarUrl = msg.author.avatarURL({ size: userRequestedSize ? userRequestedSize : defaultImageSize });
  } else {
    avatarUrl = mentionedUsers.values().next().value.avatarURL({ size: userRequestedSize ? userRequestedSize : defaultImageSize });
  }

  const embed = new MessageEmbed()
    .setTitle(`Avatar | ${mentionedUsers.size ? mentionedUsers.values().next().value.tag : msg.author.tag}`)
    .setImage(avatarUrl);
