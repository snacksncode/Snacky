...
const clearCommand = (msg: Message, args: string[], channel: TextChannel) => {
  if (!checkForPermissions(["ADMINISTRATOR"], msg.member)) {
    outputEmbedMessage(
  ...
  let numberOfMessages: number = Number(args[0]);
  ...
  channel.bulkDelete(numberOfMessages).then((messages: Collection<string, Message>) => {
    outputEmbedMessage(
